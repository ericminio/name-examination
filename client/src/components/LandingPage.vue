      ...
      <div v-else>
        ...
        <div class="card">
          <div class="card-body dashboard">
            <h2 class="dashboard">Current status on {{todayStr}}</h2>
            <table>
              <tr>
                <td class="dashboard-field">Priority requests:</td>
                <td id="priority-requests-count" class="dashboard-value">{{ dashboardData.priorityRequestCount }}</td>
              </tr>
              <tr>
                <td class="dashboard-field" colspan="2">Regular requests</td>
              <tr v-for="(record, index) in dashboardData.days" v-bind:id="'day-'+(index+1)">
                <td class="dashboard-field date indent">{{ record.date }}</td>
                <td class="dashboard-value count">{{ record.count }}</td>
              </tr>
              <tr></tr>
              <tr>
                <td class="dashboard-field indent">Total:</td>
                <td id="total" class="dashboard-value">{{ total }}</td>
              </tr>
            </table>
...
export default {
  ...
  computed: {
    ...
    total() {
      return this.$store.getters.dashboardData.days.reduce(
